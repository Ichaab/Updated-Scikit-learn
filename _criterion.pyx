...
cdef class ClassificationCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs,
                  np.ndarray[SIZE_t, ndim=1] n_classes,np.ndarray prior_dist, DOUBLE_t beta):
        ...
        self.prior_dist = <DOUBLE_t*> calloc(2, sizeof(DOUBLE_t))
        self.prior_dist = <DOUBLE_t*> prior_dist.data
        self.beta = beta
    ...
    cdef int init(self, const DOUBLE_t[:, ::1] y,
        ...
        cdef DOUBLE_t* prior_dist = self.prior_dist
        cdef DOUBLE_t beta = self.beta
...
cdef class WeightedEntropy(ClassificationCriterion):
    r"""Cross Entropy impurity criterion.
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                sum += (1/w[c]) * sum_total[c]
                #printf("w[c]=%f\nsum=%f",w[c],sum)
				
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                count_k = (1/w[c]) * sum_total[c]
                #printf("p%d=%f",c,count_k)
                if count_k > 0.0:
                    count_k /= sum 
                    entropy -= count_k * log(count_k)

            sum_total += self.sum_stride
        #printf("entropy root=%f", entropy)
        return entropy / self.n_outputs
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef double sum_le = 0.0
        cdef double sum_ri = 0.0
        cdef double sum = 0.0
        cdef SIZE_t k
        cdef SIZE_t c
        cdef double inv_weight_l = 0.0
        cdef double inv_weight_r = 0.0

        w=self.prior_dist
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                #printf("w[%d]=%f",c,(1/w[c]))
                sum_l += (1/w[c]) * sum_left[c]
                sum_r += (1/w[c]) * sum_right[c]
                sum += sum_left[c] + sum_right[c]
                sum_le += sum_left[c]
                sum_ri += sum_right[c]
        #printf("\nsum=%f",sum)
        #printf("sum_right=%f\n",sum_ri)
        ...
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                count_k = (1/w[c]) * sum_left[c]
                if count_k > 0.0:
                    count_k /= sum_l
                    #printf("count_%d = %f\n",k,count_k)
                    entropy_left -= count_k * log(count_k)
                #printf("class%d en left = %f\nentropy_left=%f",c,sum_left[c],entropy_left)

                count_k = (1/w[c]) * sum_right[c]
                if count_k > 0.0:
                    count_k /= sum_r
                    #printf("count_%d = %f\n",k,count_k)
                    entropy_right -= count_k * log(count_k)
                #printf("class%d en right = %f\nentropy_right=%f",c,sum_right[c],entropy_right)
            sum_left += self.sum_stride
            sum_right += self.sum_stride
            inv_weight_l = sum /sum_le	
            inv_weight_r = sum /sum_ri
        

        #printf("ent_right=%f\n",(sum_r / (sum_l + sum_r)) * entropy_right)
        #printf("ent_left=%f\n",(sum_l / (sum_l + sum_r)) * entropy_left)
        #printf("inv_weight_l=%f",inv_weight_l)
        #printf("output=%d",self.n_outputs)
        impurity_left[0] = (entropy_left )/ self.n_outputs
        impurity_right[0] = (entropy_right) / self.n_outputs

cdef class AECID(ClassificationCriterion):
    r"""AECID impurity criterion.
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            ...
            for c in range(n_classes[k]):
                ...
                count_k = sum_total[c]
                #w[c]=prior_dist[c]
                printf("count_k=%f et weighted_n_node_samples=%f\n ",count_k,self.weighted_n_node_samples)
                if count_k >0.0:
                    count_k/= self.weighted_n_node_samples
                    #printf("p=%f\n",count_k)
                    if(w[c]<=0.5):
                        aecid +=  ((1/((1-w[c])*w[c] **(w[c]/(1-w[c]))))*(1 - count_k)* count_k ** (w[c]/(1-w[c])))** beta
                        #aecid +=  ((1/((1-w[c])*(w[c] **(w[c]/1-w[c]))))*(1 - count_k)* (count_k ** (w[c]/(1-w[c]))))** beta
                        #printf("aecid=%f et c=%d",aecid,c)
                    else:
                        aecid +=  ((1/(w[c]*((1-w[c])**((1-w[c])/w[c]))))* count_k *((1 - count_k)** ((1-w[c])/w[c]))) ** beta
                        #printf("aecid=%f et c=%d",aecid,c)
            sum_total += self.sum_stride
            #printf("sum_total=%f",sum_total)
        #printf ("beta=%f, w[0]=%f, w[1]=%f, aecid=%f ",self.beta, w[0],w[1],aecid)

        return aecid / (self.n_outputs)
    cdef void children_impurity(self, double* impurity_left,
                                double* impurity_right) nogil:
        """Evaluate the impurity in children nodes
        ...
        cdef double* sum_right = self.sum_right
        #cdef DOUBLE_t* prior_dist = self.prior_dist
        cdef double aecid_left = 0.0
        cdef double aecid_right = 0.0
        cdef double count_k
        cdef SIZE_t k
        cdef SIZE_t c
        cdef DOUBLE_t* w = self.prior_dist
        #cdef double beta=self.beta
        cdef double beta=self.beta
        ...
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                ...
                if count_k > 0.0:
                    count_k /= self.weighted_n_left
                    if(w[c]<=0.5):
                        aecid_left += ((1/((1-w[c])*w[c] **(w[c]/(1-w[c]))))*(1 - count_k)* count_k ** (w[c]/(1-w[c])))** beta
                        #aecid_left +=  ((1/((1-w[c])*(w[c] **(w[c]/1-w[c]))))*(1 - count_k)* (count_k ** (w[c]/(1-w[c]))))** beta
                    else:
                        aecid_left += ((1/(w[c]*((1-w[c])**((1-w[c])/w[c]))))* count_k *((1 - count_k)** ((1-w[c])/w[c]))) ** beta

                count_k = sum_right[c]
                if count_k > 0.0:
                    count_k /= self.weighted_n_right
                    if(w[c]<=0.5):
                        aecid_right += ((1/((1-w[c])*w[c] **(w[c]/(1-w[c]))))*(1 - count_k)* count_k ** (w[c]/(1-w[c])))** beta
                        #aecid_right += ((1/((1-w[c])*(w[c] **(w[c]/1-w[c]))))*(1 - count_k)* (count_k ** (w[c]/(1-w[c]))))** beta
                    else:
                        aecid_right += ((1/(w[c]*((1-w[c])**((1-w[c])/w[c]))))* count_k *((1 - count_k)** ((1-w[c])/w[c]))) ** beta
                    

            sum_left += self.sum_stride
            sum_right += self.sum_stride

        impurity_left[0] = aecid_left /(self.n_outputs)
        impurity_right[0] = aecid_right / (self.n_outputs)

cdef class AE(ClassificationCriterion):
    r"""AECID impurity criterion.
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            ...
            for c in range(n_classes[k]):
                ...
                if count_k >0.0:
                    count_k/= self.weighted_n_node_samples
                    ae += (count_k *(1 - count_k))/((count_k *(1 - count_k)) + (count_k - w[c])** 2)
                                    
            sum_total += self.sum_stride
        #printf ("je suis hna ")

        return ae / ( self.n_outputs)
    cdef void children_impurity(self, double* impurity_left,
                                double* impurity_right) nogil:
        """Evaluate the impurity in children nodes
        ...
        cdef double* sum_right = self.sum_right
        #cdef DOUBLE_t* prior_dist = self.prior_dist
        cdef double ae_left = 0.0
        cdef double ae_right = 0.0
        cdef double count_k
        cdef SIZE_t k
        cdef SIZE_t c
        cdef DOUBLE_t* w = self.prior_dist
        ...
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                ...
                if count_k > 0.0:
                    count_k /= self.weighted_n_left
                    ae_left += (count_k *(1 - count_k))/((count_k *(1 - count_k))+((count_k - w[c])**2))
                    
                count_k = sum_right[c]
                if count_k > 0.0:
                    count_k /= self.weighted_n_right
                    ae_right += (count_k *(1 - count_k))/((count_k *(1 - count_k))+((count_k - w[c])**2))
                    

            sum_left += self.sum_stride
            sum_right += self.sum_stride

        impurity_left[0] = ae_left / (self.n_outputs)
        impurity_right[0] = ae_right / (self.n_outputs)

cdef class OCE(ClassificationCriterion):
    r"""Cross Entropy impurity criterion.
            ...
            for c in range(n_classes[k]):
                ...
                if count_k > 0.0:
                    ...
                    if(count_k <= w[c]):
                        ...
                        pi= (count_k + 1- (2 * w[c]))/(2 * (1 - w[c]))
                    oce -= pi * log(pi)

            sum_total += self.sum_stride

        return oce / self.n_outputs
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef double* sum_left = self.sum_left
        cdef double* sum_right = self.sum_right
        cdef double oce_left = 0.0
        cdef double oce_right = 0.0
        cdef double count_k
        cdef double pi
        cdef SIZE_t k
        cdef SIZE_t c
        ...
        impurity_left[0] = oce_left / self.n_outputs
        impurity_right[0] = oce_right / self.n_outputs
...
cdef class HellingerDistanceCriterion(ClassificationCriterion):    
    ...
    cdef void children_impurity(self, double* impurity_left,
       ...
       # printf("impurity left=%f",impurity_left[0])
       # printf("impurity right=%f",impurity_right[0])
